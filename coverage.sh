...
EXTERNAL_DIR="starsmith_tests"         # directory conatining external .miniml test files (if mode=external or both)
...
if [[ -z "$BIN" || -z "$SRC_DIR" ]]; then
    echo "Error: --bin and --src-dir are required."
    exit 1
fi
if [[ ! -f "$BIN" ]]; then
    echo "Error: binary '$BIN' does not exist."
    echo "Build with: cmake -DCODE_COVERAGE=ON -DCMAKE_CXX_COMPILER=clang++ .."
    exit 1
fi
if [[ "$MODE" == "external" || "$MODE" == "both" ]] && [[ -z "$EXTERNAL_DIR" ]]; then
    echo "Error: --external-dir is required when --mode is 'external' or 'both'."
    exit 1
fi
if [[ "$FUZZ_MODE" != "f" && "$FUZZ_MODE" != "c" ]]; then
    echo "Error: --fuzz-mode must be 'f' or 'c'."
    exit 1
fi
...
run_external_coverage() {
    ...
    shopt -s nullglob
    local files=("$EXTERNAL_DIR"/*.miniml)
    shopt -u nullglob
 
    if [[ ${#files[@]} -eq 0 ]]; then
        echo "  No .miniml files found in $EXTERNAL_DIR"
        return 1
    fi
